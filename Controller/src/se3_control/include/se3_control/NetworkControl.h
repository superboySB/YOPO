...
#include <se3_control/HGDO.h>
#include <se3_control/mavros_interface.h>
...
public:
    NetworkControl(ros::NodeHandle &node){
        ...
        nh_.param("se3_horiz_drag", se3_horiz_drag_, 0.70);
        nh_.param("se3_vert_drag", se3_vert_drag_, 0.80);
        nh_.param("se3_parasitic_drag", se3_parasitic_drag_, 0.01);
        nh_.param("se3_speed_eps", se3_speed_eps_, 1.0e-4);
        nh_.param("max_tilt_angle_deg", max_tilt_angle_deg_, 68.754935);
        nh_.param("min_thrust", min_thrust_, 2.0);
        nh_.param("max_thrust", max_thrust_, 20.0);
        nh_.param("max_ref_acc", max_ref_acc_, 6.0);
        nh_.param("max_ref_vel", max_ref_vel_, 5.0);
        nh_.param("max_ref_jerk", max_ref_jerk_, 12.0);
        nh_.param("use_reference_feedback", use_reference_feedback_, false);
        se3_controller_.setDrag(se3_horiz_drag_, se3_vert_drag_,
                                se3_parasitic_drag_, se3_speed_eps_);
        se3_controller_.setLimits(max_tilt_angle_deg_ * M_PI / 180.0,
        ...
        ROS_INFO("SE3 flatness params: mass=%.3f drag=(%.3f, %.3f, %.3f) speed_eps=%.1e tilt=%.3f deg thrust=[%.3f, %.3f]",
                 mass_, se3_horiz_drag_, se3_vert_drag_, se3_parasitic_drag_,
                 se3_speed_eps_, max_tilt_angle_deg_, min_thrust_, max_thrust_);
        ROS_INFO("YOPO trajectory execution: reference_feedback=%s max_ref=(vel %.3f, acc %.3f, jerk %.3f)",
                 use_reference_feedback_ ? "true" : "false", max_ref_vel_, max_ref_acc_, max_ref_jerk_);

        attitude_command_pub_ = nh_.advertise<quadrotor_msgs::SO3Command>("attitude_cmd", 10);
...
private:
    ...
    double se3_horiz_drag_ = 0.70, se3_vert_drag_ = 0.80, se3_parasitic_drag_ = 0.01, se3_speed_eps_ = 1.0e-4;
    double max_tilt_angle_deg_ = 68.754935, min_thrust_ = 2.0, max_thrust_ = 20.0;
    double max_ref_acc_ = 4.0;
    double max_ref_vel_ = 5.0;
    double max_ref_jerk_ = 12.0;
    ...
    bool use_reference_feedback_ = false;
    bool record_log_ = false;
    
    SE3Control se3_controller_;
    ...
    Eigen::Vector3d publishHoverSE3Command(Eigen::Vector3d des_pos, Eigen::Vector3d des_vel, Eigen::Vector3d des_acc, double des_yaw, double des_yaw_dot);

    Eigen::Vector3d pub_SE3_command(const Eigen::Vector3d &des_pos, const Eigen::Vector3d &des_vel,
    ...
    void limit_vec(Eigen::Vector3d &vec, double max_norm);
