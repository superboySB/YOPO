...
void odomToMesh(const nav_msgs::Odometry& odom_msg, 
                ...
                double color_r, double color_g, double color_b, double mesh_scale);
...
main(int argc, char** argv)
{
  ...
  double mesh_color_r, mesh_color_g, mesh_color_b, mesh_scale;
  n.param("mesh/color_r", mesh_color_r, 1.0);
  n.param("mesh/color_g", mesh_color_g, 1.0);
  n.param("mesh/color_b", mesh_color_b, 1.0);
  n.param("mesh/scale", mesh_scale, 2.0);
  ...
  while (n.ok())
  {
    ...
    if (tnow >= next_odom_pub_time)
    {
      ...
      if (mesh_pub.getNumSubscribers() > 0) {
        odomToMesh(odom_msg, meshROS, mesh_color_r, mesh_color_g, mesh_color_b, mesh_scale);
