...
def apply_config_overrides(args):
    ...
    if new_sim != sim_text:
        if args.dry_run:
            log(f"[YOPO-Omni] would update {SIM_CONFIG.relative_to(ROOT)}")
        else:
            SIM_CONFIG.write_text(new_sim)
            log(f"[YOPO-Omni] updated {SIM_CONFIG.relative_to(ROOT)}")
    if new_traj != traj_text:
        if args.dry_run:
            log(f"[YOPO-Omni] would update {TRAJ_CONFIG.relative_to(ROOT)}")
        else:
            TRAJ_CONFIG.write_text(new_traj)
            log(f"[YOPO-Omni] updated {TRAJ_CONFIG.relative_to(ROOT)}")
...
def restore_config(original_sim, original_traj, keep_config):
    ...
    if SIM_CONFIG.read_text() != original_sim:
        ...
        log(f"[YOPO-Omni] restored {SIM_CONFIG.relative_to(ROOT)}")
    if TRAJ_CONFIG.read_text() != original_traj:
        TRAJ_CONFIG.write_text(original_traj)
        log(f"[YOPO-Omni] restored {TRAJ_CONFIG.relative_to(ROOT)}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"[YOPO-Omni] command failed with exit code {exc.returncode}", file=sys.stderr, flush=True)
