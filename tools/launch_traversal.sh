...
usage() {
  cat <<'EOF'
Usage:
  tools/launch_traversal.sh [--trial N] [--epoch N] [--uav-num N] [--detach]
...
if namespace_prefix != "uav":
    raise SystemExit("Error: tools/launch_traversal.sh currently requires simulator swarm.namespace_prefix to be 'uav'.")
...
wait_for_master() {
  ...
  until rosnode list >/dev/null 2>&1; do
    if (( SECONDS >= deadline )); then
      echo "[launch_traversal] timed out waiting for roscore." >&2
      exit 1
    fi
    sleep 1
  done
...
wait_for_topic() {
  ...
  until timeout 2 rostopic echo -n 1 "$topic" >/dev/null 2>&1; do
    if (( SECONDS >= deadline )); then
      echo "[launch_traversal] timed out waiting for topic ${topic}." >&2
      exit 1
    fi
    sleep 1
  done
...
fd, out_path = tempfile.mkstemp(prefix="launch_traversal_", suffix=".rviz")
os.close(fd)
...
main() {
  ...
  if [[ "${STOP_ONLY}" -eq 1 ]]; then
    stop_all
    echo "[launch_traversal] stopped session and related processes."
    exit 0
  fi
...
PY
  ...
  if tmux has-session -t "${SESSION}" 2>/dev/null; then
    ...
    echo "Use: tools/launch_traversal.sh --session ${SESSION} --stop" >&2
    exit 1
  fi
