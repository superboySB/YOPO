...
SESSION="yopo_omni_sim"
WEIGHT="${ROOT_DIR}/YOPO/saved/YOPO_Omni_0/epoch50.pth"
PYTHON_BIN="python3"
GPU_ID="0"
VELOCITY="6.0"
MAX_DEPTH="20"
...
PLANNER_CMD="cd ${ROOT_DIR} && ${ROS_SETUP} && ${CTRL_SETUP} && ${SIM_SETUP} && PYTHONPATH=${YOPO_PYTHONPATH} CUDA_VISIBLE_DEVICES=${GPU_ID} ${PYTHON_BIN} YOPO/test_yopo_omni_ros.py --weight ${WEIGHT} --velocity ${VELOCITY} --max-depth ${MAX_DEPTH} --arrive-dist ${ARRIVE_DIST} --visualize ${VISUALIZE}"
...
RVIZ_CMD="cd ${ROOT_DIR} && ${ROS_SETUP} && export DISPLAY=${DISPLAY:-:0} && ${RVIZ_ENV} rviz --opengl ${RVIZ_OPENGL} --disable-anti-aliasing -d YOPO/yopo_omni.rviz"
