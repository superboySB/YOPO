...
def make_planner_cmd(args, exp):
    ...
    return (
        ...
        f"{shell_quote(args.python)} YOPO/test_yopo_omni_ros.py "
...
def make_monitor_cmd(args, name, output_json):
    return (
        ...
        f"cd {shell_quote(ROOT)} && "
        f"PYTHONPATH={shell_quote(yopo_pythonpath())} "
        f"{shell_quote(args.python)} tools/benchmark_yopo_omni_ros.py --monitor "
        f"--name {name} "
...
def run_monitor(args):
    ...
    rospy.init_node(f"yopo_omni_benchmark_{args.name}", anonymous=True)
...
def parse_args():
    ...
    parser.add_argument("--output-json", default="/tmp/yopo_omni_benchmark.json")
