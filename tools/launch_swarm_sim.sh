...
Usage (inside container):
  tools/launch_swarm_sim.sh [--trial N] [--epoch N] [--uav-num N] [--radius R] [--altitude Z] [--swarm-tangent-bias B] [--swarm-bias-radius R] [--detach] [--stop]
...
main() {
  ...
  if tmux has-session -t "${SESSION}" 2>/dev/null; then
    ...
    echo "Use: tools/launch_swarm_sim.sh --session ${SESSION} --stop" >&2
    exit 1
  fi
  ...
  echo "[launch_swarm] manual stop: tools/launch_swarm_sim.sh --session ${SESSION} --stop"
