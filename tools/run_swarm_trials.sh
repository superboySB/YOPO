...
usage() {
  cat <<'EOF'
Usage:
  tools/run_swarm_trials.sh [--trials N] [--uav-num N] [--timeout SEC] [--trial N] [--epoch N] [--swarm-tangent-bias B] [--swarm-bias-radius R]
...
main() {
  ...
  for run_idx in $(seq 1 "${TRIALS}"); do
    ...
    ./tools/launch_swarm_sim.sh --session "${session}" --stop >/dev/null 2>&1 || true
    sleep 1
    ./tools/launch_swarm_sim.sh \
      --trial "${YOPO_TRIAL}" \
      --epoch "${YOPO_EPOCH}" \
      --uav-num "${UAV_NUM}" \
      --radius "${RADIUS}" \
    ...
    set +e
    python3 ./tools/monitor_swarm_run.py --uav-num "${UAV_NUM}" --timeout "${TIMEOUT}" > "${result_path}"
    monitor_status=$?
    set -e
...
PY
    ...
    ./tools/launch_swarm_sim.sh --session "${session}" --stop
    sleep 2
  done
