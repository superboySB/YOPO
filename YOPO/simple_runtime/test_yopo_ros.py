...
class YopoNet:
    def __init__(self, config, weight):
        self.config = config
        rospy.init_node(self.config.get('node_name', 'yopo_simple'), anonymous=False)
        # load params
        cfg["train"] = False
        ...
        # ros publisher
        viz_prefix = self.config.get('viz_prefix', '/yopo_simple').rstrip('/')
        self.lattice_traj_pub = rospy.Publisher(viz_prefix + "/lattice_trajs_visual", MarkerArray, queue_size=1)
        self.best_traj_pub = rospy.Publisher(viz_prefix + "/best_traj_visual", Path, queue_size=1)
...
def parser():
    ...
    parser.add_argument("--node-name", default="yopo_simple")
...
if __name__ == "__main__":
    ...
    weight = args.weight or ("yopo_trt.pth" if args.use_tensorrt else
                             os.path.join(base_dir, "..", "saved", "yopo-simple", "epoch{}.pth".format(args.epoch)))
