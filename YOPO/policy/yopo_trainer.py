...
class YopoTrainer:
    ...
    def train_one_epoch(self, epoch: int, total_progress):
        ...
        traj_losses, score_losses, smooth_losses, safety_losses, goal_losses, acc_losses, bdr_losses, guard_losses, start_time = [], [], [], [], [], [], [], [], time.time()
        for step, (depth, pos, rot, obs_b, map_id) in enumerate(self.train_dataloader):  # obs: body frame
            ...
            trajectory_loss, score_loss, smooth_cost, safety_cost, goal_cost, acc_cost, bdr_cost, guard_cost = self.forward_and_compute_loss(depth, pos, rot, obs_b, map_id)
            ...
            bdr_losses.append(self.loss_weight[0] * bdr_cost.item())
            guard_losses.append(self.loss_weight[0] * guard_cost.item())
            ...
            if step % inspect_interval == inspect_interval - 1:
                ...
                self.tensorboard_log.add_scalar("Detail/BodyRateLoss", np.mean(bdr_losses), epoch * len(self.train_dataloader) + step)
                self.tensorboard_log.add_scalar("Detail/GuardLoss", np.mean(guard_losses), epoch * len(self.train_dataloader) + step)
                traj_losses, score_losses, smooth_losses, safety_losses, goal_losses, acc_losses, bdr_losses, guard_losses, start_time = [], [], [], [], [], [], [], [], time.time()
    ...
    def eval_one_epoch(self, epoch: int):
        ...
        for step, (depth, pos, rot, obs_b, map_id) in enumerate(self.val_dataloader):  # obs: body frame
            ...
            trajectory_loss, score_loss, _, _, _, _, _, _ = self.forward_and_compute_loss(depth, pos, rot, obs_b, map_id)
    ...
    def forward_and_compute_loss(self, depth, pos, rot, obs_b, map_id):
        ...
        smooth_cost, safety_cost, goal_cost, acc_cost, bdr_cost, guard_cost = self.yopo_loss(start_state_w, end_state_w, goal_w, map_id)
        trajectory_loss = (smooth_cost + safety_cost + goal_cost + acc_cost + bdr_cost + guard_cost).mean()

        score_label = (smooth_cost + safety_cost + goal_cost + acc_cost + bdr_cost + guard_cost).clone().detach()
        score_loss = F.smooth_l1_loss(score_flat, score_label)
        return trajectory_loss, score_loss, smooth_cost.mean(), safety_cost.mean(), goal_cost.mean(), acc_cost.mean(), bdr_cost.mean(), guard_cost.mean()
