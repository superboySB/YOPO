...
class YopoTrainer:
    ...
    def train_one_epoch(self, epoch: int, total_progress):
        ...
        traj_losses, score_losses, smooth_losses, safety_losses, goal_losses, acc_losses, gate_losses, start_time = [], [], [], [], [], [], [], time.time()
        for step, batch in enumerate(self.train_dataloader):  # obs: body frame
            depth, pos, rot, obs_b, map_id, gate_mask, gate_pose = self.unpack_batch(batch)
            if depth.shape[0] != self.batch_size:  continue  # batch size == number of env

            self.optimizer.zero_grad()

            trajectory_loss, score_loss, smooth_cost, safety_cost, goal_cost, acc_cost, gate_cost = self.forward_and_compute_loss(depth, pos, rot, obs_b, map_id, gate_mask, gate_pose)
            ...
            gate_losses.append(self.loss_weight[0] * gate_cost.item())
            ...
            if step % inspect_interval == inspect_interval - 1:
                ...
                self.tensorboard_log.add_scalar("Detail/GateObjectiveLoss", np.mean(goal_losses), epoch * len(self.train_dataloader) + step)
                self.tensorboard_log.add_scalar("Detail/AccelLoss", np.mean(acc_losses), epoch * len(self.train_dataloader) + step)
                self.tensorboard_log.add_scalar("Detail/GateSE3Loss", np.mean(gate_losses), epoch * len(self.train_dataloader) + step)
                traj_losses, score_losses, smooth_losses, safety_losses, goal_losses, acc_losses, gate_losses, start_time = [], [], [], [], [], [], [], time.time()
    ...
    def eval_one_epoch(self, epoch: int):
        ...
        for step, batch in enumerate(self.val_dataloader):  # obs: body frame
            ...
            trajectory_loss, score_loss, _, _, _, _, _ = self.forward_and_compute_loss(depth, pos, rot, obs_b, map_id, gate_mask, gate_pose)
    ...
    def unpack_batch(self, batch):
        if len(batch) == 5:
            depth, pos, rot, obs_b, map_id = batch
            gate_mask = torch.zeros_like(map_id, dtype=torch.float32)
            gate_pose = self.default_gate_pose(depth.shape[0], depth.device)
            return depth, pos, rot, obs_b, map_id, gate_mask, gate_pose
        if len(batch) == 6:
            depth, pos, rot, obs_b, map_id, gate_mask = batch
            gate_pose = self.default_gate_pose(depth.shape[0], depth.device)
            return depth, pos, rot, obs_b, map_id, gate_mask, gate_pose
        return batch

    def default_gate_pose(self, batch_size, device):
        pose = list(cfg["gate_pose"])
        pose[0] = float(cfg["gate_slit_roll_deg"])
        return torch.tensor(pose, dtype=torch.float32, device=device).view(1, 6).expand(batch_size, -1)

    def forward_and_compute_loss(self, depth, pos, rot, obs_b, map_id, gate_mask, gate_pose):
        depth, pos, rot, obs_b, map_id, gate_mask, gate_pose = [x.to(self.device) for x in [depth, pos, rot, obs_b, map_id, gate_mask, gate_pose]]
        ...
        gate_mask = gate_mask.repeat_interleave(self.traj_num, dim=0)  # [B*V*H]
        gate_pose = gate_pose.repeat_interleave(self.traj_num, dim=0)  # [B*V*H, 6]
        ...
        smooth_cost, safety_cost, goal_cost, acc_cost, gate_cost = self.yopo_loss(start_state_w, end_state_w, goal_w, map_id, gate_mask, gate_pose)
        trajectory_loss = (smooth_cost + safety_cost + goal_cost + acc_cost + gate_cost).mean()

        score_label = (smooth_cost + safety_cost + goal_cost + acc_cost + gate_cost).clone().detach()
        score_loss = F.smooth_l1_loss(score_flat, score_label)
        return trajectory_loss, score_loss, smooth_cost.mean(), safety_cost.mean(), goal_cost.mean(), acc_cost.mean(), gate_cost.mean()
