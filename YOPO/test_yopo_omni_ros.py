...
class YopoOmniNet:
    def __init__(self, settings, weight):
        rospy.init_node("yopo_omni_net", anonymous=False)
        cfg["train"] = False
        ...
        self.lattice_traj_pub = rospy.Publisher("/yopo_omni/topology_endstates_visual", PointCloud2, queue_size=1)
        self.best_traj_pub = rospy.Publisher("/yopo_omni/best_traj_visual", PointCloud2, queue_size=1)
        self.all_trajs_pub = rospy.Publisher("/yopo_omni/trajs_visual", PointCloud2, queue_size=1)
        self.speed_text_pub = rospy.Publisher("/yopo_omni/speed_text_visual", Marker, queue_size=1)
    ...
    def preprocess_depth(self, data):
        if data.encoding == "32FC1":
            ...
            if depth.max(initial=0.0) > 255.0:
                depth = depth / 1000.0
        else:
        ...
        if max_value <= 1.5:
            ...
            unit_msg = "already normalized"
        else:
            depth_norm = depth.astype(np.float32) / self.max_depth
            unit_msg = f"meters / {self.max_depth:g}"
    ...
    def callback_depths(self, front, left, right, back):
        ...
        time0 = time.time()
        depth = np.stack([self.preprocess_depth(msg) for msg in [front, left, right, back]], axis=0)
...
def parse_args():
    ...
    parser.add_argument("--trial", type=int, default=1, help="Trial number under YOPO/saved/YOPO_Omni_{trial}.")
...
if __name__ == "__main__":
    ...
    weight = args.weight or os.path.join(base_dir, "saved", f"YOPO_Omni_{args.trial}", f"epoch{args.epoch}.pth")
