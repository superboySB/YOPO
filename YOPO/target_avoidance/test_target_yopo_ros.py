...
class YopoNet:
    def __init__(self, config, weight):
        self.config = config
        rospy.init_node('target_avoidance_yopo', anonymous=False)
        # load params
        cfg["train"] = False
        ...
        # ros publisher
        self.lattice_traj_pub = rospy.Publisher("/target_yopo/lattice_trajs_visual", PointCloud2, queue_size=1)
        self.best_traj_pub = rospy.Publisher("/target_yopo/best_traj_visual", PointCloud2, queue_size=1)
        self.all_trajs_pub = rospy.Publisher("/target_yopo/trajs_visual", PointCloud2, queue_size=1)
        self.marker_pub = rospy.Publisher("/target/marker", Marker, queue_size=1)
        ...
        self.goal_sub = rospy.Subscriber(self.config["goal_topic"], PoseStamped, self.callback_set_goal, queue_size=1)
        # ros timer
        rospy.sleep(1.0)  # wait connection...
        self.timer_ctrl = rospy.Timer(rospy.Duration(self.ctrl_dt), self.control_pub)
        print("Target YOPO Avoidance Node Ready!")
        rospy.spin()

    def callback_set_goal(self, data):
        goal_z = data.pose.position.z if data.pose.position.z > 0.1 else self.config["default_goal_z"]
        self.goal = np.asarray([data.pose.position.x, data.pose.position.y, goal_z])
        self.arrive = False
        self.hover_goal_msg = None
        self.hover_pos = None
        print(f"New target goal: ({self.goal[0]:.1f}, {self.goal[1]:.1f}, {self.goal[2]:.1f})")
    ...
    def callback_odometry(self, data):
        ...
        if np.linalg.norm(pos - self.goal) < self.goal_tolerance and not self.arrive:
            print("Target arrive!")
            self.arrive = True
            self.hover_pos = pos.copy()
        self.publish_target_marker(data.header.stamp, pos)

    def publish_target_marker(self, stamp, pos):
    ...
    def process_odom(self):
        ...
        # goal_dir
        ref_pos = self.desire_pos if self.desire_pos is not None else np.array((
    ...
    def control_pub(self, _timer):
        ...
        if self.arrive and self.last_control_msg is not None:
            if self.hover_goal_msg is None:
                hover_pos = self.hover_pos if self.hover_pos is not None else self.goal
                self.hover_goal_msg = PositionCommand()
                self.hover_goal_msg.position.x = hover_pos[0]
                self.hover_goal_msg.position.y = hover_pos[1]
                self.hover_goal_msg.position.z = hover_pos[2]
                self.hover_goal_msg.velocity.x = 0.0
                self.hover_goal_msg.velocity.y = 0.0
                self.hover_goal_msg.velocity.z = 0.0
                self.hover_goal_msg.acceleration.x = 0.0
                self.hover_goal_msg.acceleration.y = 0.0
                self.hover_goal_msg.acceleration.z = 0.0
                self.hover_goal_msg.yaw = self.last_yaw
                self.hover_goal_msg.yaw_dot = 0.0
                self.hover_goal_msg.trajectory_flag = self.hover_goal_msg.TRAJECTORY_STATUS_EMPTY
            self.hover_goal_msg.header.stamp = rospy.Time.now()
            self.ctrl_pub.publish(self.hover_goal_msg)
            return
...
def parser():
    ...
    parser.add_argument("--weights_root", type=str, default="/workspace/YOPO/YOPO/saved/no_tracker", help="front no-tracker checkpoint root")
    parser.add_argument("--goal-x", type=float, default=16.0, help="default target goal x")
    parser.add_argument("--goal-y", type=float, default=0.0, help="default target goal y")
    parser.add_argument("--goal-z", type=float, default=2.5, help="default target goal z")
    parser.add_argument("--goal-tolerance", type=float, default=1.5, help="hover when target is this close to goal")
    return parser
...
if __name__ == "__main__":
    ...
    settings = {'use_tensorrt': args.use_tensorrt,
                ...
                'default_goal_z': args.goal_z,
                'goal_tolerance': args.goal_tolerance,
                'pitch_angle_deg': -0,   # 相机俯仰角(仰为负)
                'odom_topic': '/target/odom',
                'depth_topic': '/target_depth_image',
                'ctrl_topic': '/target_control/pos_cmd',
