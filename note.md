...
## ARM64 Deploy 镜像（Ubuntu20 + ROS Noetic）
构建（参考 deploy-ros1 用法）：
```bash
cd /home/dzp/projects/YOPO

docker build --platform=linux/arm64 -f docker/deploy.dockerfile \
  -t dzp_yopo:deploy-u2004-noetic-arm64 \
  --network=host --progress=plain .
```

运行：
```bash
docker run --name dzp-yopo-deploy -it --privileged --network host \
  -e http_proxy=http://127.0.0.1:8889 \
  -e https_proxy=http://127.0.0.1:8889 \
  -v /home/dzp/projects/YOPO:/workspace/YOPO \
  dzp_yopo:deploy-u2004-noetic-arm64
```
...
python3 -c "import torch2trt; print('torch2trt ok')"
```

说明：
- `deploy.dockerfile` 里会先执行你指定的 `pip install -U nvidia-tensorrt --index-url https://pypi.ngc.nvidia.com`。
- 若该 wheel 在当前 ARM64 平台不可用，会自动尝试 apt 的 `python3-libnvinfer` 系列包；仍不可用时不会中断构建（便于先把 ROS 环境构建出来）。
...
```bash
cd /workspace/YOPO/Controller && catkin_make

cd /workspace/YOPO/Simulator && catkin_make
```
...
docker exec -it dzp-yopo /bin/bash
```
走一个跟训练差不多的采集数据流程（这里可以换成自己定义的测试点云在`dataset`文件夹）
```sh
cd /workspace/YOPO/Simulator
source devel/setup.bash
...
然后在容器内执行：
```sh
/workspace/YOPO/tools/launch_sim.sh --trial 0 --epoch 76 --detach
```
这是基于城市地图训练的一个比较新的模型，指的就是`/workspace/YOPO/YOPO/saved/YOPO_0/epoch76.pth`
...
# 后台启动（不自动 attach到tmux窗口）
./tools/launch_sim.sh --trial xx --epoch xx --detach
